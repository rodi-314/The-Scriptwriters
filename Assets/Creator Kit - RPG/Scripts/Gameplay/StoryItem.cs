...
namespace RPGM.Gameplay
{
    ...
    public class StoryItem : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        {
            var controller = collider.GetComponent<MC_Controller>();
            if (controller != null)
            {
                ...
                {
                    if (requiredInventoryItem != null && !model.HasInventoryItem(requiredInventoryItem.name))
                        return;
                }

                foreach (var requiredStoryItem in requiredStoryItems)
                {
                    if (requiredStoryItem != null && !model.HasSeenStoryItem(requiredStoryItem.ID))
                        return;
                }
